...
summary: 'Explore React event system and its synthetic event objects for handling interactions within the UI.'
layout: PostSimple
---
...
`SyntheticEvent` （合成事件对象）这个是对浏览器原生事件对象的一层封装，兼容了主流的浏览器，同时拥有和浏览器原生事件相同的 API，例如 `stopPropagation` 和 `preventD**efault`。**`SyntheticEvent` 存在的目的就是为了消除不同浏览器在事件对象上面的一个差异**。
...
在上面的代码中，为外层的 `div` 以及内部的 `button` 都绑定了点击事件，默认情况下，点击 `button` 会打印出 `click button`、`click div`，如果打开 `e.stopPropagation( )`，那么就会阻止事件冒泡，只打印出 `click button`。

可以看出，React 内部的事件系统实现了“模拟实现事件传播机制”。

接下来自己来写一套简易版事件系统，绑定事件的方式改为 `bindXXXX`

## Implement SyntheticEvent

`SyntheticEvent` 指的是合成事件对象，在 React 中的 `SyntheticEvent` 会包含很多的属性和方法，这里出于演示的目的，只实现一个阻止冒泡
...
在上面的代码中，创建了一个 `SyntheticEvent` 类，这个类可以用来创建合成事件对象。内部保存了原生的事件对象，还提供了一个和原生 DOM 的事件对象同名的阻止冒泡的方法。
...
首先我们通过 `addEvent` 来给根元素绑定事件，目的是为了使用事件委托
...
## Collect event handling functions corresponding to the React path
...
## Implement Event Capturing and Event Bubbling
