---
title: 'Understanding the Commit Workflow in React'
date: 2024-1-24
lastmod: '2024-1-24'
tags: ['React', 'Behind The Scene', 'Tutorial']
draft: false
summary: 'Explore React commit workflow, its stages, and module breakdown for a comprehensive understanding.'
layout: PostSimple
---

# Understanding the Commit Workflow in React
...
> 相关内容回顾：
> <a href="./React-rendering-process" title="React rendering process">React rendering process</a>
...
![Understanding-the-Commit-Workflow-in-React-1](/static/images/React/Understanding-the-Commit-Workflow-in-React/Understanding-the-Commit-Workflow-in-React-1.png)
...
- 当前的 `FiberNode` 的子 `FiberNode` 不包含该子阶段(BeforeMutation, Mutation, Layout)对应的 `flags`
...
> 这段话的意思是，在 React 中，当需要在 DOM 中插入或移动元素时，使用的是`insertBefore`和`appendChild`方法。具体的行为取决于 DOM 元素的当前状态以及操作的上下文。
>
> 对于尚未插入到 DOM 中的元素（通常是`mount`场景，即首次渲染时），`insertBefore`方法会将目标 DOM 元素插入到指定的`before`元素之前。这意味着目标元素将成为`before`元素的前一个兄弟元素。
>
> 举个例子：
> 假设我们有以下 DOM 结构：
>
...
> parent.insertBefore(newElement, sibling2)
> ```
>
> 那么 DOM 结构将变为：
>
...
> ```
>
> 对于已经存在于 UI 中的 DOM 元素（通常是`update`场景，即更新渲染时），`insertBefore`方法会将目标 DOM 元素移动到指定的`before`元素之前。这意味着目标元素将从原来的位置移动到`before`元素的前一个兄弟元素的位置。
>
> 举个例子：
> 假设我们有以下 DOM 结构：
>
...
> const sibling2 = document.getElementById('sibling2')
>
> parent.insertBefore(targetElement, sibling2)
> ```
>
> 那么 DOM 结构将变为：
>
...
> ```
>
> 这是由于`insertBefore`方法的行为，它会将目标元素从原来的位置移除，并插入到指定`before`元素之前的位置。
>
> 在 React 中，这两种操作（插入和移动）都被归类为`Placement`的标记，用于标识 DOM 变更的类型。这是因为无论是插入新元素还是移动已存在元素，都涉及到将元素放置在特定位置上，而`Placement`标记正是用于表示这种放置操作的标记。
